...
1.  Data **E**xtraction: Retrieves customer data from a source.
2.  Data **T**ransformation: Cleans, scales, and encodes the data.
3.  **M**achine **L**earning: Applies a clustering model to identify patterns.
4.  Data **L**oading: Loads the processed data with pattern labels to a destination.
...
4.  **Configure Environment Variables:**
    ...
    + **Important:** Do NOT commit the `.env` file to Git, add it to .gitignore.

5.  **Set Up Database Tables if building from scratch,**
    **otherwise the tables will already be empty and in the Dockfile**
...
2.  **Ensure Dependencies Installed:** Run `pip install -r requirements.txt`
    + pandas>=1.5.0
    + supabase>=1.0.0
    + python-dotenv>=0.20.0
    + axiom-py>=0.4.0 # For Axiom logging integration
    + numpy>=1.22.0 # Often required by pandas/sklearn, good to be explicit
    + scikit-learn>=1.0.0 # For scaling, encoding, and ML models
    + matplotlib>=3.5.0 # For plotting (e.g., elbow method)
    + jupyterlab>=3.0.0 # For running the analysis notebook  
4.  **Start JupyterLab:**
    ```bash
    jupyter lab
    ```
5.  **Open Notebook:** JupyterLab will open in your web browser. Navigate to and open `snh_analysis.ipynb`.
6.  **Run Cells:** Execute the cells sequentially by selecting a cell and pressing `Shift + Enter` for `run`.
    ...
    + (Bonus) Examine the feature importances from the saved RandomForestRegressor model.
    + Fill in the final "Insights" section Markdown cell with your observations.
...
| k  | K Rate of Change | SSE      | Decrease in SSE | Ratio = (% Incr k) / (% Decr SSE) |
| :- | :--------:------- | :-------------- | :-------------------------------- |
| 2  | -==      | 1294.41  | ---             | ---                               |
| 3  | 50%      | 657.65   | 636.76          | 2.03                              |
| 4  | 33%      | 400.67   | 256.98          | 1.28                              |
| 5  | 25%      | 274.99   | 125.68          | 1.06                              |
| 6  | 20%      | 182.96   | 92.03           | **0.75**                          |
| 7  | 17%      | 146.14   | 36.82           | 0.99                              |
| 8  | 14%      | 122.85   | 23.29           | 1.05                              |
| 9  | 13%      | 90.56    | 32.29           | 0.54                              |
| 10 | 11%      | 76.60    | 13.96           | 0.81                              |
